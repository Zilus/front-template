...
gulp.task('clean-layouts', function() {
  del(config.projects[global.project].build + '*.html');
});
...
gulp.task('generate-layouts', function() {
  var fileIssues = [];
  return gulp.src(config.projects[global.project].html)
    .pipe(plugins.data(function(file) {
        return JSON.parse(fs.readFileSync(config.projects[global.project].data, 'utf8'));
      })).on('error', onError)
